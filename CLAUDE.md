...
- **Posting an external link**: route it through `features.links` (`shorten_url` for a single URL, `shorten_text` for a rendered message) so it goes through the configured Shlink instance. Both degrade to the original URL when Shlink is off or unreachable — never let a shortener failure lose a post. Currently wired into RSS entries and YouTube notifications. Deliberately *not* wired into: URLs Discord unfurls specially (`open.spotify.com` track links lose their player preview), embed image/thumbnail URLs (Discord fetches those itself — `should_shorten()` rejects media URLs), and periodically-refreshed persistent embeds (a transient Shlink outage would flip the URL back and forth and defeat `edit_message_if_changed()`).

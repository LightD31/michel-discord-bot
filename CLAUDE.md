...
`pyproject.toml` declares dependency ranges; `requirements.txt` is the lock (generated by `uv pip compile`) that the Docker image and CI install from. Edit ranges in `pyproject.toml`, then regenerate the lock.

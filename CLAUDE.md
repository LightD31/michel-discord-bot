...
- **Posting an external link**: route it through `features.links` (`shorten_url` for a single URL, `shorten_text` for a rendered message) or `src.discord_ext.links` (`shorten_embed` / `shorten_embeds` for a whole embed) so it goes through the configured Shlink instance. Every entry point degrades to the original URL when Shlink is off or unreachable — never let a shortener failure lose a post. Wired into RSS entries, YouTube notifications, the Embed manager, MDI, Streamlabs, and the Spotify/Confrérie recap messages. Periodically-refreshed embeds are safe to shorten: `shorten_url` reuses the memoized short URL and, on an API failure, falls back to the last one it minted (`cache_get(..., allow_stale=True)`), so a refresh renders an identical payload and `edit_message_if_changed()` still skips the edit. Deliberately *not* shortened: URLs Discord unfurls specially (`open.spotify.com` track links lose their player preview) and embed image/thumbnail URLs (Discord fetches those itself — `should_shorten()` rejects media URLs).

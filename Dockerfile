...
# Install from the committed lockfile (generated by `uv pip compile`, see
# CLAUDE.md) so image builds are reproducible. Copy only the lockfile first so
# the dep-install layer stays cached as long as the pins don't change.

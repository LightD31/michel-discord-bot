...
class MinecraftRCON:
    ...
    async def _receive_packet(self):
        ...
        size_data = await asyncio.get_event_loop().run_in_executor(None, self.socket.recv, 4)
        if len(size_data) < 4:
...
async def get_all_player_stats_rcon(rcon_host, rcon_port, rcon_password):
    ...
    try:
        ...
        for player in all_players:
            ...
            if (
                stats and stats.get("Temps de jeu", 0) > 0
            ):  # Seulement les joueurs avec du temps de jeu

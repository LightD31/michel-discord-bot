...
class XpExtension(Extension):
    ...
    async def on_message(self, event: MessageCreate):
        ...
        try:
            stats = await mongo_manager.get_guild_collection(guild_id, "xp").find_one(
                {"_id": user_id}
            )
    ...
    def _invalidate_rank_cache(self, guild_id: str) -> None:
        ...
        keys_to_delete = [
            key for key in list(self._rank_cache._cache.keys()) if key.startswith(f"{guild_id}_")
        ]
    ...
    async def _handle_level_up(
        ...
        await mongo_manager.get_guild_collection(guild_id, "xp").update_one(
            {"_id": user_id}, {"$set": {"lvl": new_level}}, upsert=True
        )
    ...
    async def _get_user_rank(self, guild_id: str, user_id: str) -> int | None:
        ...
        try:
            ...
            result = (
                await mongo_manager.get_guild_collection(guild_id, "xp")
                .aggregate(pipeline)
                .to_list(length=None)
            )
    ...
    async def _get_user_rank_fallback(self, guild_id: str, user_id: str) -> int | None:
        ...
        try:
            rankings = (
                mongo_manager.get_guild_collection(guild_id, "xp")
                .find({}, {"_id": 1})
                .sort("xp", -1)
            )
            rank = 0
    ...
    async def _build_leaderboard_embeds(self, guild: Guild) -> list[Embed]:
        ...
        try:
            rankings_cursor = (
                mongo_manager.get_guild_collection(str(guild.id), "xp").find().sort("xp", -1)
            )

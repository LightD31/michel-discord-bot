...
from features.birthday import BirthdayEntry, BirthdayRepository, _safe_replace_year, _strip_year_from_format
...
class BirthdayExtension(Extension):
    ...
    async def anniversaire(
        ...
        try:
            ...
            logger.info(
                ...
                ctx.author.display_name, verb, ctx.guild.name, parsed_date.strftime("%d/%m/%Y"),
            )
    ...
    async def anniversaire_timezone(self, ctx: AutocompleteContext) -> None:
        ...
        filtered = [tz for tz in all_tz if timezone_input in tz.lower()][:25] if timezone_input else random.sample(all_tz, 25)
    ...
    async def anniversaire_purge(self, ctx: SlashContext) -> None:
        confirm_button = Button(style=ButtonStyle.DANGER, label="Confirmer la suppression", custom_id="birthday_purge_confirm")
        cancel_button = Button(style=ButtonStyle.SECONDARY, label="Annuler", custom_id="birthday_purge_cancel")
        msg = await ctx.send(
        ...
        try:
            button_ctx: ComponentContext = await self.bot.wait_for_component(
                components=[confirm_button, cancel_button], messages=msg, timeout=30,
            )
            if button_ctx.custom_id == "birthday_purge_confirm":
                total_deleted = sum(
                    await self._repo(gid).delete(ctx.author.id) for gid in enabled_servers
                )
                await button_ctx.edit_origin(content=f"Anniversaire supprimé sur {total_deleted} serveur(s) ✅", components=[])
    ...
    async def _handle_birthday_celebration(self, entry: BirthdayEntry, guild_id: int, now_tz: datetime) -> None:
        if entry.isBirthday:
            return
        ...
        try:
            ...
            logger.info("Birthday celebration completed for %s on server %s (%d years old)", member.display_name, server.name, age)
        except Exception as e:
    ...
    async def _send_birthday_message(self, channel, member, age: int, server_id: int) -> None:
        try:
            ...
            text = pick_weighted_message(
                srv_cfg, "birthdayMessageList", "birthdayMessageWeights",
